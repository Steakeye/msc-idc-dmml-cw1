...
what <- unlist(tail_deltas, recursive=FALSE)
...
attributes_order <- attributes[correct_order]
ordered_outlier_vals <- tail_deltas[correct_order]

listOfList <- list(attributes, tail_deltas)

dataframe <- data.frame(attributes = attributes, tail_neighbour_d=I(tail_deltas))
dataframe$tail_neighbour_d
#order(dataframe$tail_neighbour_d)
#dataframe[ ]
#setNames(do.call(rbind.data.frame, listOfList), c("col1", "col2"))


ordered <- table(unlist(listOfList))
asframe <- as.data.frame(ordered)
colnames(asframe) <- c("attributes", "outlier val")
