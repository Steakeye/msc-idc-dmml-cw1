...
While this new set of data is now has no values beyond the outermost quartile ranges, this is argaubly too harsh a treatment. Boxplot has an outlier property that we can use to collect values that we might want to remove, so this is the one option we will look at for cleansing data.
...
iqr_outliers <- c()
quartlie_multiplier = 5

# Loop through the list of columns you specified
for(i in worst_outliers){

  # Get the Min/Max values
  max <- quantile(wine.scaled_cleansed_iqr[,i],0.75, na.rm=FALSE) + (IQR(wine.scaled_cleansed_iqr[,i], na.rm=FALSE) * quartlie_multiplier )
  min <- quantile(wine.scaled_cleansed_iqr[,i],0.25, na.rm=FALSE) - (IQR(wine.scaled_cleansed_iqr[,i], na.rm=FALSE) * quartlie_multiplier )
...
number_of_clusters <- NbClust(wine.scaled_cleansed_bp,
                 min.nc=2, max.nc=15,
                 method="kmeans")
...
sse_list <- 0
for (i in 1:15){
  sse_list[i] <- sum(kmeans(wine.scaled_cleansed_bp, centers=i)$withinss)
}
...
```{r}
fit.km <- kmeans(wine.scaled_cleansed_bp, 2)
