...
Before we can test the neural network against test data we need to transform the test data we took from the origina dataset and transform it so we can pass in three input variables like when the network was trained.
...
```{r testing-mlp-f1}
mlp.nn1_results <- compute(mlp.nn1, testdata)
```
