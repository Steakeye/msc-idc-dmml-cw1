...
Before we can test the neural network against test data we need to transform the test data we took from the original dataset and transform it so we can pass in three input variables similarly to how the network was trained.
...
test_expected_data <- tail(exchange.scaled_test$`USD/EUR`, -3)

#ls(mlp.nn1_results)

#print(mlp.nn1_results$net.result)
test_v_expected <- cbind(test_expected_data, as.data.frame(head(mlp.nn1_results$net.result, -1)))
colnames(test_v_expected) <- c("Expected Output", "Neural Net Output")
print(test_v_expected)
```

And then the Sum of Square Errors, and Mean Squared Error values can be derived in order to look at the performance of the trained nural network against test data.
...
test_delta <- output_delta_func(test_v_expected$`Expected Output`, test_v_expected$`Neural Net Output`)
