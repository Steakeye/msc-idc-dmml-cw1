...
### {x, x} hidden layer structure

```{r}
#mlp.form1 <- as.formula("Output ~ Input_d0 + Input_dneg1 + Input_dneg2")

mlp.nn_x_x <- neuralnet(mlp.form1, normalised_staggered_data_frame, hidden = c(4,2), threshold=0.25, stepmax = 1200000)
```
...
```{r}
mlp.nnX_results <- compute(mlp.nn_x_x, normalised_staggered_test_data_frame)

test_expected_data.nnX <- tail(exchange.normalised_test$`USD/EUR`, -3)

test_v_expected.nnX <- expected_v_test_func(test_expected_data.nnX, mlp.nnX_results)
```
```{r}
test_delta.nnX <- output_delta_func(test_v_expected.nnX$`Expected Output`, test_v_expected.nnX$`Neural Net Output`)
#SSE of this first nn
t_sse <- sse_func(test_delta.nnX)
t_sse
#MSE of this first nn
t_mse <- mse_func(test_delta.nnX)
t_mse
#RMSE for the nn
t_rmse <- rmse(test_v_expected.nnX$`Neural Net Output`, test_v_expected.nnX$`Expected Output`)
t_rmse
#MAPE for the nn
t_mape <- percent(mape(test_v_expected.nnX$`Neural Net Output`, test_v_expected.nnX$`Expected Output`))
t_mape
...
```{r}
# Create Line Chart
linetypes <- line_compare_nn_to_actual(test_v_expected.nnX, test_v_expected.nnX$`Expected Output`, suffix = "(NNX)")

# add a legend 
legend(0, .75, names(test_v_expected.nnX), cex=0.8, col=linetypes[[3]], pch=linetypes[[2]], lty=linetypes[[1]], title="Exchange Rates")
```