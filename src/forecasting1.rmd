...
```{r line-graph-mlp-f1}
# Create Line Chart
line_compare_nn_to_actual(test_v_expected.nn1, test_v_expected.nn1$`Expected Output`)

# add a legend 
legend(45, -.25, names(test_v_expected.nn1), cex=0.8, col=colors, pch=plotchar, lty=linetype, title="Exchange Rates")
```

What's intersting about looking at it visually, is how it's more obvious from this perspective than it is from indicators values, that the neural net predictions are similar overall but with a latency of a few days; this isn't very useful for forecasting and might actually suggest a kind of overfitting, where the network has been too tightly trained with teh training data. I'm not certain that this is the case however because this graph is based on data that was not used for training purpose.

## Experimenting with other Neural Network configucations

Now thgat we have one neural net with performance results, it's time to use this as a benchmark/baseline and create other neural networks to see if we can improve on performance.
...
test_expected_data.nn1 <- tail(exchange.scaled_test$`USD/EUR`, -3)mlp.nn1_nonl_results <- compute(mlp.nn1_nonl, staggered_test_data_frame)
...
The hidden layers are the 'secret sauce' of this kind of neural network, the layers describe nuerons consisting of nodes and vertices that connect to other neuron nodes; the verteces between nodes ajust the values as they are passed from node to node resulting in the anwer at the end of the chain off layers. The first neural network had 3 hidden layers each powers of 2, starting with 8 and halving at each next layer.

The next step is to experiment with the same dataset and the same input values but experiment with the accuracy of some different hinder layer stuctures like:
...
mlp.nn_6_9_6 <- neuralnet(mlp.form1, staggered_data_matrix, hidden = c(6, 9, 6), threshold=0.05, stepmax = 800000)
```
...
# Create Line Chart
line_compare_nn_to_actual(test_v_expected.nn2, test_v_expected.nn2$`Expected Output`)

# add a legend 
legend(45, -.25, names(test_v_expected.nn2), cex=0.8, col=colors, pch=plotchar, lty=linetype, title="Exchange Rates")
```
...
mlp.nn_6_9_6_3 <- neuralnet(mlp.form1, staggered_data_matrix, hidden = c(6, 9, 6, 3), threshold=0.05, stepmax = 800000)
```
...
# Create Line Chart
line_compare_nn_to_actual(test_v_expected.nn3, test_v_expected.nn3$`Expected Output`)

# add a legend 
legend(45, -.25, names(test_v_expected.nn3), cex=0.8, col=colors, pch=plotchar, lty=linetype, title="Exchange Rates")
```
